...
> ![Note]
> This tool is developed on and for mac machines. Some features might not work properly on other platforms because of platform specific terminal command.
...
1. Download the latest stable prebuilt binary from [releases]().
...
> (running `pwd` returns the current path)
